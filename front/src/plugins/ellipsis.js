  ...
  Vue.component("VueEllipsis", {
    ...
    methods: {
      ...
      handleSubstrSentence() {
        ...
        html = this.htmlScape(this.htmlScape(html).trim());
        ...
        const stNodeLineHeight = this.lineHeight.slice(
          0,
          this.lineHeight.length - 2
        );
        let stNodeHeight =
        ...
        if (stNodeHeight > maxHeight) {
          ...
          const endStr = this.endHtml
            ? this.endHtml.replace(/<[^>]+>/g, "")
            : "";
          const endLen =
          ...
          const stNodeDelStr = stNode.innerHTML.substring(
            stNodeLen - endLen,
            stNodeLen
          );
          ...
          const newHtml =
            stNode.innerHTML.substring(0, stNodeLen - endLen - extraLen) +
            this.endChar +
            this.endHtml;
          stNode.innerHTML = this.htmlScape(newHtml);
        }
      },
      handleClick(e) {
        this.$emit("click", e);
